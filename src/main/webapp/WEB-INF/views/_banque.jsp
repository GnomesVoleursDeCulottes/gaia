        ...
        <td id="quantiteAEchange" class="longueurCaseBanque">
            <select id="transaction">
                <c:if test="${joueur.quantiteBle > 3}">
                    <option value="blevschevre">Donner 4 blés contre une chèvre</option>
                </c:if>
                <c:if test="${joueur.quantiteCarotte > 1}">    
                    <option value="carottevschevre">Donner 2 carottes contre une chèvre</option>
                </c:if>
                <c:if test="${nbChevres > 0}">    
                    <option value="chevrevsble">Donner 1 chèvre contre 4 blés</option>
                </c:if>
                <c:if test="${nbChevres > 0}">    
                    <option value="chevrevscarotte">Donner 1 chèvre contre 2 carottes</option>
                </c:if>    
            </select>
        </td>
